...
public class RemoteUsageRegistryHandler extends AbstractRemoteStoreHandler {
  ...
  public void registerDataset(HttpRequest request, HttpResponder responder) throws ClassNotFoundException {
    List<MethodArgument> arguments = parseArguments(request);

    Id.Program programId = deserialize(arguments.get(0));
  ...
  @POST
  @Path("/registerStream")
  public void registerStream(HttpRequest request, HttpResponder responder) throws ClassNotFoundException {
    List<MethodArgument> arguments = parseArguments(request);

    Id.Program programId = deserialize(arguments.get(0));
    Id.Stream streamId = deserialize(arguments.get(1));
