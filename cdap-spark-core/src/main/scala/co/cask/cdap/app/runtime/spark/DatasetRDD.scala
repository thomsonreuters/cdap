...
class DatasetRDD[K: ClassTag, V: ClassTag](@transient sc: SparkContext,
  ...
  override protected def getPartitions: Array[Partition] = {
    if (delegateRDD.isEmpty) {
      delegateRDD = Some(datasetCompute(datasetNamespace, datasetName, arguments, (dataset: Dataset) => {
        ...
        dataset match {
          case batchReadable: BatchReadable[K, V] => {
            new BatchReadableRDD[K, V](sc, batchReadable, datasetNamespace, datasetName, arguments, splits,
              txServiceBaseURI)
          }
