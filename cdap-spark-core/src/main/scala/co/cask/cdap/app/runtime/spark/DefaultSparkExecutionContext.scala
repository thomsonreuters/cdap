...
class DefaultSparkExecutionContext(runtimeContext: SparkRuntimeContext,
  ...
  override def fromDataset[K: ClassTag, V: ClassTag](sc: SparkContext,
    ...
    new DatasetRDD[K, V](sc, createDatasetCompute, runtimeContext.getConfiguration, None, datasetName, arguments,
      splits, getTxServiceBaseURI(sc, sparkTxService.getBaseURI))
  }
  ...
  override def fromDataset[K: ClassTag, V: ClassTag](sc: SparkContext,
                                                     ...
                                                     arguments: Map[String, String],
                                                     splits: Option[Iterable[_ <: Split]]): RDD[(K, V)] = {
    new DatasetRDD[K, V](sc, createDatasetCompute, runtimeContext.getConfiguration, Some(datasetNamespace),
  ...
  private def createDatasetCompute: DatasetCompute = {
    new DatasetCompute {
      override def apply[T: ClassTag](datasetNamespace: Option[String], datasetName: String,
        ...
        transactional.execute(new SparkTxRunnable {
          override def run(context: SparkDatasetContext) = {
            val dataset: Dataset = if (datasetNamespace.isEmpty) {
              context.getDataset(datasetName, arguments, AccessType.READ)
            } else {
              context.getDataset(datasetNamespace.get, datasetName, arguments, 
                AccessType.READ)
            }
